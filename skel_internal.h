...
static inline int bpf_load_and_run(struct bpf_load_and_run_opts *opts)
{
    ...
    printk(KERN_INFO "[%s] map created fd: %d\n", __FUNCTION__, map_fd); 
    ...
    printk(KERN_INFO "[%s] map updated\n", __FUNCTION__); 
	...
	if (prog_fd < 0) {
        printk(KERN_INFO "[%s] program load err: %d\n", __FUNCTION__, prog_fd); 
		opts->errstr = "failed to load loader prog";
		set_err;
		goto out;
	}
    printk(KERN_INFO "[%s] program loaded fd: %d\n", __FUNCTION__, prog_fd); 
    ...
    printk(KERN_INFO "[%s] program run... err: %d\n", __FUNCTION__, err); 
